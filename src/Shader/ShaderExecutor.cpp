...
void ShaderExecutor::update(){
    ...
    mFboPingpong.dst->getTexture().getTextureData().bFlipTexture = true;
    ...
    if(!mAudioFileExist && mGLProgramAudio && mAudGen) {
        ...
        if(!wasExecuted) {
            static int blocks_ctr = 0;
            {
                ...
                mAudGen->writeToBuf(offset, mAudioPixels.getData());
                blocks_ctr += 1;
            }
        ...
        if(!mSoundPlayer.isPlaying()) {
            string wav_rel_path = "";
            mSoundPlayer.load(wav_rel_path.append("Cache/Wav/").append(mAudGen->getRawname()).append(".wav"));
